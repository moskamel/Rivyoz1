...
import { INCOME_CATEGORIES } from '../../constants/arabic';

type CategoryKey = keyof typeof INCOME_CATEGORIES;
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.background }}>
      <View style={{
        ...
        <Text weight="bold" size="lg">إضافة إيراد</Text>
      ...
      <ScrollView contentContainerStyle={{ padding: Spacing.base, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 10, marginBottom: 24 }}>
          {(Object.entries(INCOME_CATEGORIES) as [CategoryKey, { label: string; icon: string }][]).map(([key, val]) => (
            <Pressable
              key={key}
            ...
            >
              ...
              <Text size="sm" weight={category === key ? 'semiBold' : 'regular'}
                color={category === key ? Colors.primary : Colors.textSecondary} align="center">
                {val.label}
              </Text>
            </Pressable>
          ))}
