...
function PinMap({ restaurants, selected, onSelect }) {
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden', background: '#e8f0e4' }}>
      <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.55 }}>
        {[20, 35, 50, 65, 80].map(y => <rect key={`h${y}`} x="0" y={`${y}%`} width="100%" height="6" fill="#fff" />)}
        {[15, 30, 48, 65, 82].map(x => <rect key={`v${x}`} x={`${x}%`} y="0" width="6" height="100%" fill="#fff" />)}
      ...
      {restaurants.map(r => {
        ...
        return (
          <div key={r.id} onClick={() => onSelect(isSelected ? null : r)}
            style={{ position: 'absolute', left: `${r.x}%`, top: `${r.y}%`, transform: 'translate(-50%, -100%)', cursor: 'pointer', zIndex: isSelected ? 20 : 10, transition: 'transform 0.15s' }}>
            <div style={{ width: isSelected ? 54 : 44, height: isSelected ? 54 : 44, borderRadius: '50%', background: r.isOpen ? r.color : '#9CA3AF', border: `3px solid ${isSelected ? 'white' : 'rgba(255,255,255,0.8)'}`, boxShadow: isSelected ? `0 6px 20px ${r.color}80, 0 0 0 4px ${r.color}30` : `0 3px 10px rgba(0,0,0,0.22)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: isSelected ? 26 : 20, opacity: r.isOpen ? 1 : 0.55, transition: 'all 0.2s' }}>
              {r.emoji}
            </div>
          ...
          <div style={{ padding: '16px 16px 0' }}>
            ...
            {!search && activeFilter === 'الكل' && filterTab === 'الكل' && (
              <div style={{ marginBottom: 24 }}>
                <p style={{ fontSize: 14, fontWeight: 800, color: 'var(--text)', marginBottom: 12 }}>⭐ مميزون</p>
                <div style={{ display: 'flex', gap: 12, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 4 }}>
                  {featured.map(r => (
                    <div key={r.id} onClick={() => navigate(`/${r.slug}`)}
                      style={{ flexShrink: 0, width: 200, height: 140, borderRadius: 16, overflow: 'hidden', cursor: 'pointer', background: r.imageUrl ? `url(${r.imageUrl}) center/cover no-repeat` : `linear-gradient(135deg, ${r.color}, ${r.color}bb)`, position: 'relative', boxShadow: `0 8px 24px ${r.color}40`, transition: 'transform 0.15s, box-shadow 0.15s' }}
            ...
            {filtered.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '48px 24px' }}>
                ...
                <p style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)', marginBottom: 6 }}>لم نجد نتائج</p>
                <p style={{ fontSize: 13, color: 'var(--text-3)' }}>جرب كلمة بحث مختلفة</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {filtered.map(r => (
                  <div key={r.id} onClick={() => navigate(`/${r.slug}`)}
                    style={{ background: 'var(--surface)', borderRadius: 16, overflow: 'hidden', cursor: 'pointer', border: '1px solid var(--border)', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', opacity: r.isOpen ? 1 : 0.7, transition: 'transform 0.15s, box-shadow 0.15s', display: 'flex', alignItems: 'stretch' }}
                    ...
                    <div style={{ flex: 1, padding: '12px 14px 12px 12px', minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 3 }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <p style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 3 }}>{r.category}</p>
                          {r.address && (
                            <div style={{ display: 'flex', alignItems: 'center', gap: 3, marginBottom: 4 }}>
                              <Location size={10} color="var(--text-3)" />
                              <span style={{ fontSize: 10, color: 'var(--text-3)' }}>{r.address}</span>
                            </div>
                          )}
                        </div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                          <Clock size={11} color="var(--text-3)" />
                          <span style={{ fontSize: 11, color: 'var(--text-2)' }}>{r.deliveryTime} دقيقة</span>
                        </div>
                        <span style={{ color: 'var(--border)', fontSize: 10 }}>·</span>
                        <span style={{ fontSize: 11, color: r.deliveryFee === 0 ? 'var(--green)' : 'var(--text-2)', fontWeight: r.deliveryFee === 0 ? 700 : 400 }}>
