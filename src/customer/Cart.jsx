...
import { ArrowRight, TickCircle, CloseCircle, HambergerMenu } from 'iconsax-react'
...
function StepIndicator({ step, color }) {
  const steps = [{ num: 1, label: 'التوصيل' }, { num: 2, label: 'الدفع' }, { num: 3, label: 'التأكيد' }]
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '12px 20px 4px' }}>
      {steps.map((s, idx) => (
        <div key={s.num} style={{ display: 'flex', alignItems: 'center', flex: idx < steps.length - 1 ? 1 : 'none' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, flexShrink: 0 }}>
            <div style={{
              ...
              background: s.num < step ? '#22C55E' : s.num === step ? color : 'var(--border-strong)',
              color: s.num <= step ? 'white' : 'var(--text-3)',
              transition: 'all 0.3s',
            }}>
              {s.num < step ? <TickCircle size={14} /> : s.num}
            </div>
            <span style={{
              fontSize: 10, fontWeight: 700, fontFamily: 'Zain, sans-serif', whiteSpace: 'nowrap',
              color: s.num === step ? color : s.num < step ? '#22C55E' : 'var(--text-3)',
              transition: 'color 0.3s',
            }}>{s.label}</span>
          </div>
          {idx < steps.length - 1 && (
            <div style={{ flex: 1, height: 2, background: s.num < step ? '#22C55E' : 'var(--border)', margin: '0 6px', marginBottom: 20, minWidth: 20, transition: 'background 0.3s' }} />
          )}
        </div>
      ))}
    </div>
...
export default function Cart() {
  ...
  /* step: 0=cart  1=details  2=payment  3=review */
  ...
  const validateStep1 = () => {
    ...
    else if (!/^(010|011|012|015)\d{8}$/.test(phone.replace(/\s|-/g, ''))) e.phone = 'رقم غير صحيح (010/011/012/015 + 8 أرقام)'
  ...
  const NextBtn = ({ onClick, label, disabled }) => (
    <button
      ...
      style={{
        ...
        boxShadow: disabled ? 'none' : `0 6px 20px ${color}40`,
        transition: 'all 0.2s',
      }}
    >{label || 'متابعة'}</button>
  )
  ...
  const BackBtn = ({ to }) => (
    <button onClick={() => goToStep(to)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 0', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-2)', fontWeight: 700, fontSize: 14, fontFamily: 'Zain, sans-serif' }}>
  ...
  const Header = () => (
    <div style={{
      ...
      {step > 0 && (
        <button onClick={() => goToStep(step - 1)} style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--surface-2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
          <ArrowRight size={18} color="var(--text-2)" />
        </button>
      )}
      <h1 style={{ flex: 1, fontWeight: 800, fontSize: 18, color: 'var(--text)', margin: 0 }}>
        {step === 0 ? `السلة${itemCount > 0 ? ` (${itemCount})` : ''}` : step === 1 ? 'بيانات التوصيل' : step === 2 ? 'طريقة الدفع' : 'مراجعة وتأكيد'}
      </h1>
      {step === 0 && (
        <button onClick={() => setSidebarOpen(true)} style={{ width: 36, height: 36, borderRadius: 10, background: 'var(--surface-2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'Zain, sans-serif', direction: 'rtl' }}>
      <Header />
      {step > 0 && <StepIndicator step={step} color={color} />}

      <div style={{ padding: `${step > 0 ? 12 : 16}px 14px 100px`, display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <>
                ...
                {orderTypeOpts.map(opt => (
                  <button key={opt.key} onClick={() => opt.enabled && setOrderType(opt.key)} disabled={!opt.enabled}
                    style={{
                      ...
                      border: `2px solid ${orderType === opt.key ? color : 'var(--border-strong)'}`,
                      background: orderType === opt.key ? color + '12' : 'var(--surface-2)',
                      color: orderType === opt.key ? color : 'var(--text-2)',
                      opacity: opt.enabled ? 1 : 0.4, fontFamily: 'Zain, sans-serif',
                    }}>
            ...
            <Card>
              ...
              <div style={{ padding: '0 16px 16px' }}>
                <NextBtn onClick={() => canProceed && goToStep(1)} disabled={!canProceed} label="متابعة لتفاصيل التوصيل" />
              </div>
            </Card>
          </>
        )}
          ...
          <>
            <Card>
              ...
              <div style={{ padding: '12px 16px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div>
                  ...
                  <input type="text" value={name} onChange={e => { setName(e.target.value); setStep1Errors(p => ({ ...p, name: '' })) }}
                    placeholder="اسمك الكامل"
                    style={{ ...baseInput, borderColor: step1Errors.name ? '#EF4444' : 'var(--border-strong)' }}
                    onFocus={e => focusInput(e, color)} onBlur={blurInput} />
                  <ErrorMsg msg={step1Errors.name} />
                </div>
                <div>
                  <Label error={step1Errors.phone}>رقم الهاتف *</Label>
                  <input type="tel" value={phone} dir="ltr" onChange={e => { setPhone(e.target.value); setStep1Errors(p => ({ ...p, phone: '' })) }}
                    placeholder="01XXXXXXXXX"
                    style={{ ...baseInput, borderColor: step1Errors.phone ? '#EF4444' : 'var(--border-strong)', fontFamily: 'Inter, sans-serif' }}
                    onFocus={e => focusInput(e, color)} onBlur={blurInput} />
              ...
              <Card>
                ...
                <div style={{ padding: '12px 16px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <select value={governorate} onChange={e => { setGovernorate(e.target.value); setStep1Errors(p => ({ ...p, governorate: '' })) }}
                      style={{ ...baseInput, borderColor: step1Errors.governorate ? '#EF4444' : 'var(--border-strong)', appearance: 'none' }}
                      onFocus={e => focusInput(e, color)} onBlur={blurInput}>
                  ...
                  <div>
                    ...
                    <textarea value={address} onChange={e => { setAddress(e.target.value); setStep1Errors(p => ({ ...p, address: '' })) }}
                      placeholder="الحي / المنطقة، الشارع، رقم المبنى..." rows={3}
                      style={{ ...baseInput, resize: 'none', borderColor: step1Errors.address ? '#EF4444' : 'var(--border-strong)' }}
                      onFocus={e => focusInput(e, color)} onBlur={blurInput} />
            ...
            {orderType === 'table' && (
              <Card>
                ...
                <div style={{ padding: '12px 16px 16px' }}>
                  <input type="number" value={tableNumber} onChange={e => { setTableNumber(e.target.value); setStep1Errors(p => ({ ...p, tableNumber: '' })) }}
                    placeholder="مثال: 5"
                    style={{ ...baseInput, borderColor: step1Errors.tableNumber ? '#EF4444' : 'var(--border-strong)', fontFamily: 'Inter, sans-serif' }}
                    onFocus={e => focusInput(e, color)} onBlur={blurInput} />
            ...
            <NextBtn onClick={() => { if (validateStep1()) goToStep(2) }} label="متابعة لاختيار الدفع" />
          </>
        )}
        ...
        {step === 2 && (
          <>
                ...
                {payOpts.map(opt => (
                  <button key={opt.key} onClick={() => setPayment(opt.key)}
                    style={{
                      ...
                      border: `2px solid ${payment === opt.key ? color : 'var(--border-strong)'}`,
                      background: payment === opt.key ? color + '08' : 'var(--surface)',
            ...
            {/* Totals strip — compact, no repetition */}
            <div style={{ background: 'var(--surface)', borderRadius: 14, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 13, color: 'var(--text-2)', fontWeight: 600 }}>الإجمالي المستحق</span>
          ...
          <>
              ...
              {cartItems.map((item, idx) => (
                <div key={item.cartId} style={{ padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: idx < cartItems.length - 1 ? '1px solid var(--border)' : 'none' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {item.modifiers?.length > 0 && (
                      <p style={{ fontSize: 11, color: 'var(--text-3)', marginTop: 2 }}>{item.modifiers.map(m => m.name).filter(Boolean).join('، ')}</p>
                    )}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 12, color: 'var(--text-3)', fontFamily: 'Inter, sans-serif' }}>× {item.qty}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color, fontFamily: 'Inter, sans-serif' }}>{item.price * item.qty} ج</span>
                  </div>
                </div>
              ))}
            </Card>
            ...
            {orderType === 'delivery' && address && (
              <Card>
                <div style={{ padding: '14px 16px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <p style={{ fontWeight: 700, fontSize: 13, color: 'var(--text)', marginBottom: 4 }}>عنوان التوصيل</p>
                    {governorate && <p style={{ fontSize: 12, color, fontWeight: 700, marginBottom: 2 }}>{governorate}</p>}
                    <p style={{ fontSize: 13, color: 'var(--text-2)', lineHeight: 1.6 }}>{address}</p>
                    {driverNote && <p style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 3 }}>ملاحظة: {driverNote}</p>}
                  </div>
                </div>
              </Card>
            )}
            ...
            <Card>
              <div style={{ padding: '14px 16px', display: 'flex', gap: 10, alignItems: 'center' }}>
                <span style={{ fontSize: 22 }}>{payOpts.find(p => p.key === payment)?.icon}</span>
                <div>
                  <p style={{ fontWeight: 700, fontSize: 13, color: 'var(--text)', marginBottom: 2 }}>طريقة الدفع</p>
                  <p style={{ fontSize: 13, color: 'var(--text-2)' }}>{payOpts.find(p => p.key === payment)?.label}</p>
                </div>
              </div>
            </Card>
            ...
            <Card>
              ...
              <div style={{ padding: '12px 16px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: 'var(--text-2)' }}>
                  <span>رسوم التوصيل</span>
                  <span style={{ fontFamily: 'Inter, sans-serif' }}>{orderType === 'delivery' ? `${config.deliveryFee} ج` : 'مجاني'}</span>
                </div>
